...
local keyboard_flags = 0

MOUSE_CODE_MAPPING = {
    [512] = 0, -- mouse move
    [513] = 1, -- left btn down
    [514] = 2, -- left btn up
    [516] = 3, -- FIXME right btn down
    [517] = 4, -- FIXME right btn up
    [519] = 10, -- FIXME middle btn down
    [520] = 11, -- FIXME middle btn up
    [522] = 12, -- FIXME scroll wheel
    [523] = 20, -- FIXME xtra btn down
    [524] = 21, -- FIXME xtra btn up
}
        ...
        if event == EVENT_LOAD and #handlers_by_event[EVENT_LOAD] == 1 then
            ashita.register_event('load', function()
                for _, handler in pairs(handlers_by_event[EVENT_LOAD]) do
                    ...
                    if not success then
                        windower.error('Load handler: ' .. msg)
                    end
                end
            end)
        end
        ...
        if event == EVENT_PRERENDER and #handlers_by_event[EVENT_PRERENDER] == 1 then
            ashita.register_event('prerender', function()
                for _, handler in pairs(handlers_by_event[EVENT_PRERENDER]) do
                    local success, msg = pcall(handler)
                    if not success then
                        windower.error('Prerender handler: ' .. msg)
                    end
                end
            end)
        end
...
--                    print('modify packet ... id=' .. tostring(id))
        ...
        if event == EVENT_KEYBOARD and #handlers_by_event[EVENT_KEYBOARD] == 1 then
            ashita.register_event('key', function(key, down, blocked)
--                print('key code is '.. key .. ', down is ' .. dump(down))
                local block = blocked or false

                if key == 42 or key == 54 then -- lshift and rshift
                    if down then
                        keyboard_flags = bit.bor(keyboard_flags, 0x01)
                    else
                        keyboard_flags = bit.band(keyboard_flags, bit.bnot(0x01))
                    end
                end
                if key == 56 or key == 184 then -- lalt and ralt
                    if down then
                        keyboard_flags = bit.bor(keyboard_flags, 0x02)
                    else
                        keyboard_flags = bit.band(keyboard_flags, bit.bnot(0x02))
                    end
                end
                if key == 29 or key == 157 then -- lctrl and rctrl
                    if down then
                        keyboard_flags = bit.bor(keyboard_flags, 0x04)
                    else
                        keyboard_flags = bit.band(keyboard_flags, bit.bnot(0x04))
                    end
                end
                if key == 219 or key == 220 then -- lwin and rwin
                    if down then
                        keyboard_flags = bit.bor(keyboard_flags, 0x08)
                    else
                        keyboard_flags = bit.band(keyboard_flags, bit.bnot(0x08))
                    end
                end
                for _, handler in pairs(handlers_by_event[EVENT_KEYBOARD]) do
                    local res = handler(key, down, keyboard_flags, block)
                    if res ~= nil then
                        block = block or res
                    end
                end
        ...
        if event == EVENT_MOUSE and #handlers_by_event[EVENT_MOUSE] == 1 then
            ashita.register_event('mouse', function(id, x, y, delta, blocked)
--                print('mouse id is '.. id)
                local block = blocked or false

                local type = MOUSE_CODE_MAPPING[id]
                for _, handler in pairs(handlers_by_event[EVENT_MOUSE]) do
                    local res = handler(type, x, y, delta, block)
                    if res ~= nil then
                        block = block or res
                    end
                end
...
local change_handlers = {
    ...
    ['target'] = function() return AshitaCore:GetDataManager():GetTarget():GetTargetIndex() end,
    ['zone']   = function() return AshitaCore:GetDataManager():GetParty():GetMemberZone(0) end,
    ['job']    = function() return {
                                AshitaCore:GetDataManager():GetPlayer():GetMainJob(),
                                AshitaCore:GetDataManager():GetPlayer():GetMainJobLevel(),
                                AshitaCore:GetDataManager():GetPlayer():GetSubJob(),
                                AshitaCore:GetDataManager():GetPlayer():GetSubJobLevel(),
                            } end,
}
...
local handle_change_events = function()
    for name, value_fn in pairs(change_handlers) do
        ...
        if handlers_by_event[event] and #handlers_by_event[event] > 0 then
            ...
            local new = value_fn()
            if name == 'job' then
                if not table.equals(old, new, 1) then
                    for _, handler in pairs(handlers_by_event[event]) do
                        -- print('job change triggered')
                        handler(unpack(new))
                    end
                end
            else
...
local keybinds = T{}
local execute_bind_command = function(cmd)
    --print('execute_bind_command: ' .. cmd)
    local chat_open, modifier, key, line = string.match(cmd, '(%%?)([!^~]?)([^ ]*)[ ]+(.*)')
    ...
    if not keybinds:find(modifier .. key) then
        keybinds:insert(modifier .. key)
    end
    local bind = '/bind ' .. modifier .. key .. ' /' .. line
    --print(bind)
    AshitaCore:GetChatManager():QueueCommand(bind, 1)
end

local execute_unbind_command = function(cmd)
    --print('execute_unbind_command: ' .. cmd)
    local chat_open, modifier, key = string.match(cmd, '(%%?)([!^~]?)([^ ]*)')
    if key == nil or #key == 0 then
        return
    end
    if modifier == '~' then
    ...
    local unbind = '/unbind ' .. modifier .. key
    --print(unbind)
    AshitaCore:GetChatManager():QueueCommand(unbind, 1)
end

local unbind_all = function()
    for _, combo in pairs(keybinds) do
        local unbind = '/unbind ' .. combo
        --print(unbind)
        AshitaCore:GetChatManager():QueueCommand(unbind, 1)
    end
end
...
--    print('SEND COMMAND: ' .. command)
    ...
    elseif string.sub(first, 1, 5) == 'bind ' and #first > 5 then
        execute_bind_command(string.trim(string.sub(first, 5)))
    elseif string.sub(first, 1, 7) == 'unbind ' and #first > 7 then
        execute_unbind_command(string.trim(string.sub(first, 7)))
    else
...
windower.console = windower.console or {}

windower.console.write = function(text)
    print(text)
end

windower.chat = windower.chat or {}

windower.chat.input = function(text)
    execute_input_command(text)
end
...
windower.packets.parse_action = function(packet)
    ...
    for i = 1, res.target_count do
        ...
        for n = 1, target.action_count do
            ...
            action['animation']                  = ashita.bits.unpack_be(packet, offset+5,  12)
            action['effect']                     = ashita.bits.unpack_be(packet, offset+17,  4)
            action['stagger']                    = ashita.bits.unpack_be(packet, offset+21,  3)
            action['knockback']                  = ashita.bits.unpack_be(packet, offset+24,  3)
...
--    print('windower.text.get_extents')
    ...
    return text:GetBackground():GetWidth(), text:GetBackground():GetHeight()
end
...
--    moon_phase: int
--    time: int
    res['zone'] = AshitaCore:GetDataManager():GetParty():GetMemberZone(0)
--    mog_house: bool
    res['logged_in'] = logged_in
    res['menu_open'] = false -- TODO
    res['chat_open'] = AshitaCore:GetChatManager():IsInputOpen()
--    weather: int
    res['language'] = 'en'
    res['server'] = 31 -- TODO everyone is on Asura anyway, right?
    res['target_arrow'] = {
--        x: number
--        y: number
--        z: number
...
windower.ffxi.get_items = function(bag, index)
    local res = {}
    res['safe'] = {
--        ['1'] = {
--            ['count'] = 0,
...
--        ['2'] = {},
    ...
    res['count_case'] = 0
    res['count_satchel'] = 0
    res['count_inventory'] = 0
    res['count_storage'] = 0
    res['count_wardrobe'] = 0
    res['count_temporary'] = 0
    res['enabled_safe'] = 0
    res['enabled_locker'] = 0
    res['enabled_sack'] = 0
    res['enabled_case'] = 0
    res['enabled_satchel'] = 0
    res['enabled_inventory'] = 0
    res['enabled_storage'] = 0
    res['enabled_wardrobe'] = 0
    res['enabled_temporary'] = 0
    res['equipment'] = {
--        ['ammo'] = 0,
--        ['back'] = 0,
--        ...etc.
...
--        print 'id not found'
    ...
    res['is_npc'] = (bit.band(entity.SpawnFlags, 0x1) == 0)
    res['entity_type'] = entity.EntityType
    res['spawn_type'] = entity.SpawnFlags
    ...
    res['in_party'] = (bit.band(entity.SpawnFlags, 0x4) > 0)
    res['in_alliance'] = (bit.band(entity.SpawnFlags, 0x8) > 0)
    res['valid_target'] = (bit.band(entity.SpawnFlags, 0x10) > 0)
--    print(dump(res))
    return res
end
...
--    linkshell_slot: int
    ...
    res['main_job_level'] = AshitaCore:GetDataManager():GetPlayer():GetMainJobLevel()
--    main_job_full: string
    res['main_job_id'] = AshitaCore:GetDataManager():GetPlayer():GetMainJob()
--    superior_level: int
--    item_level: int
--    nation: int
    res['id'] = entity.ServerId
    res['status'] = entity.Status
    res['index'] = entity.TargetIndex
    res['sub_job'] = jobs[AshitaCore:GetDataManager():GetPlayer():GetSubJob()] or 'NONE'
    res['sub_job_level'] = AshitaCore:GetDataManager():GetPlayer():GetSubJobLevel()
--    sub_job_full: string
    res['sub_job_id'] = AshitaCore:GetDataManager():GetPlayer():GetSubJob()
    res['target_index'] = 0
...
windower.ffxi.get_ability_recasts = function()
    local res = {}
    for index = 0, 31 do
        local id   = ashita.ffxi.recast.get_ability_id_from_index(index)
        local time = ashita.ffxi.recast.get_ability_recast_by_index(index)
        if time > 0 then
            res[id] = time / 60
        end
    end
...
windower.ffxi.get_spell_recasts = function()
    ...
    for index = 0, 1024 do
        local id   = index
        local time = ashita.ffxi.recast.get_spell_recast_by_index(index)
        if time > 0 then
            res[id] = time
        end
    end
